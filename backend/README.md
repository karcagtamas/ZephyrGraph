...
For the settings, you need to build the graph-model library with gradle: `gradle build` for project `backend:libs:graph-model` and the result `graph-model-2.3.0.jar` from the project's `/build/libs` folder.
...
```cmd
@echo off
echo %HOMEPATH%\.config\kotlin-language-server\lib\graph-model-2.3.0.jar
```

And place the built `graph-model-2.3.0.jar` next to the bat file into a `lib` folder.
...
```bash
#!/bin/bash
echo ~/.config/kotlin-language-server/lib/graph-model-2.3.0.jar
```
