...
```cmd
@echo off
echo %HOMEPATH%\.config\kotlin-language-server\lib\graph-model-1.2.0.jar
```

And place the built `graph-model-1.2.0.jar` next to the bat file into a `lib` folder.
...
```bash
#!/bin/bash
echo /~/.config/kotlin-language-server/lib/graph-model-1.2.0.jar
```
