...
For the settings, you need to build the graph-model library with gradle: `gradle build` for project `backend:libs:graph-model` and the result `graph-model-2.1.0.jar` from the project's `/build/libs` folder.
