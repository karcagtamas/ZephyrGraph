...
A cause-effect graph maps the interdependencies between these inputs and outputs, where each connection between a cause and an effect represents a logical relationship, often described in terms of \textbf{Boolean} logic. These logical relationships are typically expressed using basic logical operators such as \textbf{AND}, \textbf{OR}, and \textbf{NOT}, which define how different causes combine to produce specific effects.

\subsection{Key Components of a Cause-Effect Graph}

\subsubsection{Nodes}

The nodes represent the causes and the effects of the graph. Each node in the graph can be represented as a Boolean expression and can either be true or false, reflecting the presence or absence of a condition or outcome.

\subsubsection{Edges}

Directed edges connect causes to effects, illustrating the logical dependency between them. The edges represent the conditions under which an effect occurs, based on the combination of the cause inputs.

\subsubsection{Logical Operators}

The relationships between causes and effects are typically governed by logical operators. Mostly three of them are used:
...
Constraints may be applied to causes and effects, such as mutually exclusive conditions or specific combinations of causes that must or must not occur together.
...
The figure \ref{fig:basic-cause-effect} shows a basic visual representation of a graph with three nodes: two cause nodes connected to an effect node through an AND relationship.
...
\begin{itemize}
	\item \textbf{AND}: The effect occurs only if all connected causes are true. For example, if $ C1 \land C2 = E1 $, the effect E1 will only happen when both C1 and C2 are true.
	\item \textbf{OR}:The effect occurs if at least one of the connected causes is true. For instance, $ C1 \lor C2 = E1 $ indicates that E1 will occur if either C1 or C2 is true.
	\item \textbf{NOT}: The effect occurs if the connected cause is false. For example, $ \lnot C1 = E1 $ means that E1 will happen if C1 is false.
\end{itemize}
...
In the visual representation, we achieve a similar result to the figure \ref{fig:basic-cause-effect}. As noted earlier, the primary difference is the inclusion of an additional graph node for the logical operator. The details are shown in the figure \ref{fig:proposed-syntax-cause-effect}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\textwidth,height=100px]{ProposedSyntaxGraph}
	\caption{Visual representation of the proposed syntax on the figure \ref{fig:basic-cause-effect}}
	\label{fig:proposed-syntax-cause-effect}
\end{figure}
...
\subsubsection{Evaluation of Causes}
...
\subsubsection{Effect Determination}
...
If the conditions for an effect are met (the causes and logical relationships evaluate to true), the effect is triggered or considered true. If not, the effect remains false.

\subsubsection{Propagation of Truth Values}
...
\subsubsection{Handling Complex Conditions}
...
For example, if the graph contains a complex condition such as $ ((C1 \land C2) \lor C3) = E1 $, the truth value of $ (C1 \land C2) $ is first evaluated before combining it with \textbf{C3} to determine whether E1 is true.

\subsubsection{Boolean Logic Interpretation}
...
In the first example, there are only two causes and one effect. This small graph simulates a basic login access scenario: if the user enters valid credentials and has an active account, the system will grant access. These nodes are listed in the table \ref{tab:graph-example-1} and the graph visualization are shown in the figure \ref{fig:graph-example-1}.
...
Both cause, \textbf{C1} and \textbf{C2}, must be true for the effect, \textbf{E1}, to occur. The logical relationship can be expressed as: $ C1 \land C2 = E1 $.
...
The various colors in the figure \ref{fig:graph-example-1} indicate the different types of nodes in the graph.

\subsection{Example 2}

In the second example, the structure is more complex, primarily due to nested relationships. This case includes five nodes: three causes and two effects. With two effects, we can create two graph slices. One slice uses the three causes with AND connections, while the other is similar but negates the \textbf{C3} node.
...
\textbf{E1} only occurs if \textbf{C1}, \textbf{C2}, and \textbf{C3} are all true. However, \textbf{E2} occurs if \textbf{C1} and \textbf{C2} are true, but \textbf{C3} is false.

The semantic representation is as follows:

\begin{compactitem}
	\item $ (C1 \land C2 \land C3) = E1 $
	\item $ (C1 \land C2 \land \lnot C3) = E2 $
\end{compactitem}

As with the previous example, we can create a visualized graph. For convenience, the two slices are separated into smaller graphs.
