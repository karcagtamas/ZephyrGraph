...
\begin{enumerate}
	...
	\item \textbf{Effect Determination}
		\begin{itemize}
			...
			\item If the conditions for an effect are met (i.e., the causes and logical relationships evaluate to true), the effect is triggered or considered true. If not, the effect remains false.
		\end{itemize}
	\item \textbf{Propagation of Truth Values}
		...
		\begin{itemize}
			\item \textbf{AND}: The effect is true only if all connected causes are true. For example, if the causes C1 and C2 are both true, then the effect E1 ($ C1 \land C2 \rightarrow E1 $) will also be true.
			\item \textbf{OR}: The effect is true if at least one of the connected causes is true. For instance, if either C1 or C3 is true, the effect E2 ($ C1 \lor C3 \rightarrow E2 $) will be true.
			\item \textbf{NOT}: The effect is true if the connected cause is false. For example, if C4 is false, then the effect E3 ($ \lnot C4 \rightarrow E3 $) will be true.
		\end{itemize}
	\item \textbf{Handling Complex Conditions}
		\begin{itemize}
			...
			\item For example, if the graph contains a complex condition such as $ ((C1 \land C2) \lor C3) \rightarrow E1 $, the truth value of $ (C1 \land C2) $ is first evaluated before combining it with C3 to determine whether E1 is true.
		\end{itemize}
	\item \textbf{Boolean Logic Interpretation}
		...
		\begin{itemize}
			\item $ (C1 \land C2) \rightarrow E1 $ translates to a Boolean expression where C1 and C2 implicate E1.
			\item $ \lnot C4 \rightarrow E3 $ means that C4 implicates E3.
		\end{itemize}	
\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabular}{ | m{0.1\textwidth} | m{0.1\textwidth} | m{0.65\textwidth} | }
		...
		\hline
	\end{tabular}
	\caption{Example 1 - Nodes}
	\label{tab:graph-example-1}
\end{table}

Both cause, C1 and C2, must be true for the effect, E1, to occur. The logical relationship can be expressed as: $ C1 \land C2 \rightarrow E1 $
...
\begin{compactitem}
	\item $ (C1 \land C2 \land C3) \rightarrow E1 $
	\item $ (C1 \land C2 \land \lnot C3) \rightarrow E1 $
\end{compactitem}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth,height=100px]{Example_2_1}
	\caption{Example 2 - Graph Rule 1}
	\label{fig:graph-example-2-1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth,height=100px]{Example_2_2}
	\caption{Example 2 - Graph Rule 2}
